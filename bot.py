...
class GetProduct(BaseModel):
    ...
    place: str = Field('', description="the user is ready to buy or place an order for the product, e.g. готов купить, хорошо оставлю заявку, давайте оформим")
    ...
    if user_state == "place_order":
        ...
        if tool_calls and isinstance(tool_calls, list) and len(tool_calls) > 0:
            ...
            if last_product['name'] != '' and name != last_product['name']:
                ...
                compression_class = args['compression_class'] if 'compression_class' in args and args['compression_class'] != '' else None
